...
implementations against each other. **CORRECTED 2026-07-31 — the 2026-07-30 correction was itself unbacked.** The figures published here on 2026-07-30 (`1,698.7 ms -> 245.7 ms, a 6.82x median (range 6.65-6.98x)`, `7 paired reps`) appear in no artifact, and the arithmetic does not close: 1,698.7 / 245.7 = **6.91**, not 6.82. The quoted band was also *narrower than every measured shape* — the same n-too-small error the 6.18x figure had already been retracted for. Replaced with the committed harness output (`make bench-fast-path`, `artifacts/crs/bench_fast_path.json`), **11 paired reps** per shape, counts bit-identical on every rep: on the densest shape **1,491.2 ms -> 244.6 ms**, and across all three shapes a **median of 6.06x, observed 3.55x-10.25x**. Quote the median; the range moves with machine load and problem shape.
